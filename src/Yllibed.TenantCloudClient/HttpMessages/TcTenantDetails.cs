...
public partial class TcTenantDetails
{
	...
	public string?[] ValidEmails
	{
		get
		{
			...
			{
				if (IsValidEmail(Email1, out var email1))
				{
					yield return email1;
				}

				if (IsValidEmail(Email2, out var email2))
				{
					yield return email2;
				}

				if (IsValidEmail(Email3, out var email3))
				{
					yield return email3;
				}
			}
	...
	public string[] ValidPhones
	{
		get
		{
			...
			{
				if (IsValidPhone(Phone1, out var phone1))
				{
					yield return phone1!;
				}

				if (IsValidPhone(Phone2, out var phone2))
				{
					yield return phone2!;
				}

				if (IsValidPhone(Phone3, out var phone3))
				{
					yield return phone3!;
				}
			}
	...
	private static bool IsValidEmail(string? s, out string? output)
	{
		output = null;
	...
	[GeneratedRegex(
		@"(?:\+)?(?:\d[\-\s\(\)]?){8,15}",
		RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.CultureInvariant | RegexOptions.ExplicitCapture)]
	private static partial Regex PhoneRegex();

	private static bool IsValidPhone(string? s, out string? output)
	{
		output = null;
