...
public class InMemoryTcContext : ITcContext
{
	...
	public InMemoryTcContext(Func<CancellationToken, Task<(string username, string password)>> asyncCredentialsCallback)
	{
		...
		{
			var (username, password) = await asyncCredentialsCallback(ct).ConfigureAwait(false);
	...
	public Task<string?> GetAuthToken(CancellationToken ct) => Task.FromResult(_token);
}
