...
internal class PaginatedSource<T> : IPaginatedSource<T>
{
	...
	public async Task<ReadOnlySequence<T>> GetAll(CancellationToken ct, long maxResults = 300)
	{
		var pageNo = 1;
		...
		{
			var (entries, _, totalEntries) = await GetPage(ct, pageNo++).ConfigureAwait(false);
